@echo off
REM Like run-harness-ab but connects to a DEAD port so the client stays in the
REM login scene (where MU_TEST_CHARS renders the crowd). Args: <blendinst> <waveinst> <tag>
setlocal
set "REL=%~dp0out\build\windows-x86\src\Release"
cd /d "%REL%" || (echo No se encontro %REL% & exit /b 1)
del /q gl_log.txt 2>nul
del /q harness_shot.jpg 2>nul
set "MU_TEST_CHARS=100"
set "MU_TEST_SHOT=600"
set "MU_GPUBMD=1"
set "MU_GPUINST=0"
set "MU_GPUSHADOW=1"
set "MU_GPUSKIN=1"
set "MU_GPUBLENDMESH=1"
set "MU_GPUBLENDINST=%~1"
set "MU_GPUWAVEINST=%~2"
set "MU_NOVSYNC=1"
set "MU_FPS=1"
echo [dead] blendinst=%~1 waveinst=%~2 tag=%~3
start "" "%REL%\Main.exe" connect /u127.0.0.1 /p44999
set /a tries=0
:wait
if exist harness_shot.jpg goto done
set /a tries+=1
if %tries% GTR 120 ( echo [dead] TIMEOUT no shot & taskkill /F /IM Main.exe >nul 2>nul & copy /y gl_log.txt "gl_log_%~3.txt" >nul & exit /b 2 )
ping -n 2 127.0.0.1 >nul
goto wait
:done
ping -n 5 127.0.0.1 >nul
taskkill /F /IM Main.exe >nul 2>nul
copy /y gl_log.txt "gl_log_%~3.txt" >nul
copy /y harness_shot.jpg "shot_%~3.jpg" >nul
echo [dead] done tag=%~3
endlocal
