...
namespace Core::Diagnostics
{
    ...
    void TemporalCsvLogger::EnsureHeader()
    {
        ...
        if (m_out.is_open())
            m_out << "t_ms,fps,hero_x,hero_y,hero_render_x,hero_render_y,hero_units_per_sec,steps,interp_alpha,frame_ms,hero_anim,hero_anim_render,eff_step,eff_decay,cpu_render_ms,swap_ms,terrain_ms,objects_ms,chars_ms,effects_ms\n";
    ...
    void TemporalCsvLogger::LogFrame(double timeMs, double fps, float heroX,
                                     ...
                                     double terrainMs, double objectsMs,
                                     double charsMs, double effectsMs)
    {
        if (!m_enabled)
            return;
        ...
        m_out << timeMs << ',' << fps << ',' << heroX << ',' << heroY << ','
              ...
              << effStep << ',' << effDecay << ','
              << cpuRenderMs << ',' << swapMs << ','
              << terrainMs << ',' << objectsMs << ',' << charsMs << ',' << effectsMs << '\n';
    }
}
