@echo off
REM run-harness-deadport with a configurable char count (no MU_JOBS).
...
del /q gl_log.txt harness_shot.jpg 2>nul
set "CHARS=%~4"
...
set "MU_NOVSYNC=1"
set "MU_FPS=1"
echo [dead] blendinst=%~1 waveinst=%~2 tag=%~3 chars=%CHARS%
